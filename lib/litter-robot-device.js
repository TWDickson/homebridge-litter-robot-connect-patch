...
class LitterRobotDevice {
    ...
    setOccupancyService(service) {
        ...
        this.occupancyService.getCharacteristic(platform.Characteristic.OccupancyDetected)
            .onGet(async () => {
                const value = device.getOccupancy();
                return value;
              });
        this.occupancyService.getCharacteristic(platform.Characteristic.OccupancyDetected)
            .updateValue(async () => {
    ...
    setTrayService(service) {
        const platform = this.platform;
        const device = this;

        this.trayService = service;
        this.trayService.getCharacteristic(platform.Characteristic.OccupancyDetected)
            .on('get', async (callback) => {
                platform.log('Tray state requested');
                let value = this.getFilterChange();
                callback(null, value);
            });
        this.trayService.getCharacteristic(platform.Characteristic.StatusActive)
            .on('get', async (callback) => {
                platform.log('Tray status state requested');
                let value = device.getOccupancyFault();
                callback(null, !value);
            });
        this.trayService.getCharacteristic(platform.Characteristic.StatusFault)
            .on('get', async (callback) => {
                platform.log('Tray status state requested');
                let value = device.getOccupancyFault();
                callback(null, value);
